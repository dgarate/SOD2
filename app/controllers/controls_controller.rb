class ControlsController < ApplicationController
  ...
  # POST /controls.json
  def create
    @control = Control.new(name: control_params[:name], description: control_params[:description], end_date: control_params[:end_date])

    respond_to do |format|
      if @control.save
          control_params[:cycle_ids].reject(&:empty?).each_with_index do |id, index|
          responsibility_id = control_params[:responsibility_ids].reject(&:empty?)
          company_id = control_params[:company_ids].reject(&:empty?)
          Conflict.create(control_id: @control.id, cycle_id: id, responsibility_id: responsibility_id[index], company_id: company_id[index])
      end
  ...
  private
    ...
    def control_params
      params.require(:control).permit(:name, :description, :end_date, company_ids:[], responsibility_ids:[], cycle_ids:[])
    end
end
